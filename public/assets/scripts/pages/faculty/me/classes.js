...
function Grades() {
    ...
    menuItems.forEach(item => {
        item.addEventListener('click', () => {
            ...
            // Show corresponding grades container
            const subject = item.dataset.subject;
            const container = document.querySelector(`.subject-grades[data-subject="${subject}"]`);
            container.classList.add('active');
        });
    });

    // Initialize grading platform for each subject
    gradeContainers.forEach(container => {
        ...
        if (platformContainer) {
            SelectModel(sectionSubjectId, "SECTION_SUBJECT_CONTROL").then((res) => {
                SelectModelByFilter(JSON.stringify({section_id: res.section_id}), "SECTION_STUDENT_CONTROL").then((students) => {
